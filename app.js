...
btnStart.addEventListener("click", async () => {
  try {
    await startCamera();
    running = true;
    statusEl.textContent = "Camera ready. Start Scanning…";
    speak("Camera ready. Start Scanning…");
    requestAnimationFrame(loop);
  } catch (e) {
...
btnStop.addEventListener("click", () => {
  running = false;
  statusEl.textContent = "Camera has stop scanning";
  speak(" Camera has stop scanning");
});
...
function updateOverlayText(color) {
  ...
  overlay.textContent = `Scanning for ${colorName} QR...`;
}

// Example: if you’re using radio buttons for color
document.querySelectorAll('.color-slider input[type="radio"]').forEach(input => {
  input.addEventListener('change', () => {
    updateOverlayText(input.value);
  });
});
